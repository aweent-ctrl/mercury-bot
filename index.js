...
async function createTemporaryVoiceChannel(eventItem) {
  ...
  try {
    ...
    const parentId =
      textChannel && textChannel.parentId ? textChannel.parentId : null;
...
function scheduleTenMinuteActions(eventItem) {
  ...
  console.log(
    `Событие ${getEventTitle(eventItem)} на ${
      eventItem.timeText
    }. Напоминание через ${Math.round(delay / 1000)} секунд.`
  );
...
async function sendHelp(message) {
  return message.reply(
    [
      ...
      "**!limit число** — изменить лимит в обычной стреле.",
...
client.on("messageCreate", async (message) => {
  try {
    ...
    console.log("Ошибка messageCreate:", error);
  }
});
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    console.log("Ошибка interactionCreate:", error);

    try {
      if (interaction.deferred || interaction.replied) {
        await interaction.editReply({
          content: "Произошла ошибка при выполнении действия. Проверь консоль бота.",
        });
      } else {
        await interaction.reply({
          content: "Произошла ошибка при выполнении действия. Проверь консоль бота.",
...
client.on("voiceStateUpdate", async (oldState) => {
  ...
  if (oldChannel.members.size === 0) {
    ...
    await oldChannel.delete("Приватная комната пуста").catch(() => {});
  }
});
